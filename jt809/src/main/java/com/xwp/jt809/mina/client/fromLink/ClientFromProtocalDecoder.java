...
public class ClientFromProtocalDecoder extends CumulativeProtocolDecoder {

	private final Charset charset;
	private final AttributeKey CONTEXT = new AttributeKey(getClass(),"context");

	public ClientFromProtocalDecoder() {
		this(Charset.defaultCharset());
	}

	public ClientFromProtocalDecoder(Charset charset) {
		...
		while(in.hasRemaining()){
			...
			if(ctx.isMark() && b == 0x5d){
				...
				if(blist.size() == 30){
					...
					System.out.println("未识别消息长度："+blist.size());
				}
				ctx.reset();
				return true;
			}else if(b == 0x5b){
